...
const createNewBook = async function (req,res) {
    ...
    const Saved = await bookModel1.create(reqBook)
    res.send( {msg : Saved})
...
const allBooks = async function(req, res) {
    ...
    const booksName = await bookModel1.find({author_id: id}).select({name:1})
    res.send( {msg:booksName})
}

const upadatedBookPrice = async function (req, res) {
    const bookDetails = await bookModel1.find({name:"Two states"})
    ...
    const updatedPrice = await bookModel1.findOneAndUpdate({name:bkName}, {price:100},{new:true}).select({price:1, _id:0})
...
const authorsName = async function (req,res) {
    const booksId= await bookModel1.find({price: {$gte:50, $lte:100}}).select({author_id:1, _id:0})
